...
describe('GRUP Handler', () => {
  ...
  describe('Top-Level GRUP (type 0)', () => {
    it('should process a PERK GRUP correctly', () => {
      ...
      const recordHeader = createRecordHeader('PERK', subrecord.length, 0x12345678);
      ...
      expect(records[0].data.EDID[0].toString('ascii')).toBe('TestPerk\0');
    });
  ...
  describe('Nested GRUP', () => {
    it('should process nested GRUPs correctly', () => {
      ...
      const subrecord = createSubrecord('EDID', edidData);
      const recordHeader = createRecordHeader('PERK', subrecord.length, 0x87654321);
      const recordBuffer = Buffer.concat([recordHeader, subrecord]);
      ...
      expect(records[0].data).toHaveProperty('EDID');
      expect(records[0].data.EDID[0].toString('ascii')).toBe('NestedPerk\0');
    });
    ...
    it('should handle multiple records in nested GRUP', () => {
      ...
      const nestedGrupBuffer = Buffer.concat([nestedGrupHeader, record1, record2]);
      ...
      expect(records[0].data.EDID[0].toString('ascii')).toBe('Perk1\0');
      expect(records[1].data.EDID[0].toString('ascii')).toBe('Perk2\0');
    });
  });
